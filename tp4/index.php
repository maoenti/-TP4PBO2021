...
if(isset($_POST['add']))
{
	...
	$otask->addProject($tname, $tdetails, $tsubject, $tpriority, $tdeadline);
	header("Location: index.php");
}

while (list($id, $tname, $tdetails, $tsubject, $tpriority, $tdeadline, $tstatus) = $otask->getResult())
{
		if($tstatus == "Sudah Bayar"){
			$data .= "<tr>
			<td>" . $no . "</td>
			<td>" . $tname . "</td>
			<td>" . $tdetails . "</td>
		...
		// Tampilan jika status task nya belum dikerjakan
			...
			<td>
			<button class='btn btn-danger'><a href='index.php?id_hapus=" . $id . "' style='color: white; font-weight: bold;'>Hapus</a></button>
			<button class='btn btn-success' ><a href='index.php?id_status=" . $id .  "' style='color: white; font-weight: bold;'>Bayar</a></button>
			</td>
			</tr>";
			$no++;
		}
